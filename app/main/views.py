...
def model_predict():
    loader = Loader(settings.MODEL_PATH, settings.WORD_TO_INDEX_DICT_PATH, settings.INDEX_TO_WORD_DICT_PATH)
    model_predictor = Model_Predictor(loader, settings.UNKNOWN_TOKEN, settings.START_TOKEN, settings.SEQUENCE_LENGTH)
    ...
    #return render_template('model_template.html', form=form, name=session.get('sa_model_output'))
    return render_template('model_template.html', form=form, name='model in development. Please try again later')
...
def sensitivity_analysis():    
    ...
    form_choices = [(x.replace('.pickle','') , x.replace('.pickle','')) for x in df_filenames]
    ...
    plotter = Plotter(df_plot_input, settings.SENSITIVITY_PLOT_TITLE, settings.PARAM_FEATURE_NAME, settings.ACC_FEATURE_NAME)
    plot_sens = plotter.create_plot()    
    script, div = components(plot_sens)
    return render_template('sensitivity_analysis.html', form = form, script = script, div = div)
